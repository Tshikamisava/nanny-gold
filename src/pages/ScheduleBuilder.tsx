...
const ScheduleBuilder = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-purple-50 to-yellow-50 px-6 py-8">
      <div className="max-w-sm mx-auto">
        ...
        {!isEmergencyBooking && selectedDates.length > 0 && <div className="bg-white rounded-xl royal-shadow p-4 mb-6">
            ...
            <div className="mb-4">
              ...
              <input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full p-3 border border-purple-200 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-200" min="06:00" max="20:00" />
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="space-y-3">
                <label className="flex items-center">
                  <input type="radio" name="breakPreference" value="3-in-1-out" checked={breakPreference === '3-in-1-out'} onChange={e => setBreakPreference(e.target.value)} className="w-4 h-4 text-purple-600 border-purple-300 focus:ring-purple-500" />
                  <span className="ml-3 text-sm text-purple-900">3 weekends in, 1 weekend out</span>
                </label>
                
                <label className="flex items-center">
                  <input type="radio" name="breakPreference" value="2-in-2-out" checked={breakPreference === '2-in-2-out'} onChange={e => setBreakPreference(e.target.value)} className="w-4 h-4 text-purple-600 border-purple-300 focus:ring-purple-500" />
                  <span className="ml-3 text-sm text-purple-900">2 weekends in, 2 weekends out</span>
                </label>
                
                <label className="flex items-center">
                  <input type="radio" name="breakPreference" value="1-in-3-out" checked={breakPreference === '1-in-3-out'} onChange={e => setBreakPreference(e.target.value)} className="w-4 h-4 text-purple-600 border-purple-300 focus:ring-purple-500" />
                  <span className="ml-3 text-sm text-purple-900">1 weekend in, 3 weekends out</span>
                </label>
                
                <label className="flex items-center">
                  <input type="radio" name="breakPreference" value="all-weekends-out" checked={breakPreference === 'all-weekends-out'} onChange={e => setBreakPreference(e.target.value)} className="w-4 h-4 text-purple-600 border-purple-300 focus:ring-purple-500" />
        ...
        {isEmergencyBooking && <div className="bg-white rounded-xl royal-shadow p-4 mb-6">
            ...
            {timeSlots.map((slot, index) => <div key={index} className="flex items-center space-x-2 mb-4">
                <div className="flex-1">
                  ...
                  <input type="time" value={slot.start} onChange={e => updateTimeSlot(index, 'start', e.target.value)} className="w-full p-2 border border-purple-200 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-200" min="00:00" max="23:59" />
                </div>
                <div className="flex-1">
                  <label className="block text-sm font-medium text-purple-900 mb-1">End Time</label>
                  <input type="time" value={slot.end} onChange={e => updateTimeSlot(index, 'end', e.target.value)} className="w-full p-2 border border-purple-200 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-200" min="00:00" max="23:59" />
                </div>
            ...
            {timeSlots.length === 0 && <Button variant="outline" onClick={addTimeSlot} className="w-full border-purple-300 text-purple-600 hover:bg-purple-50">
                Add Time Slot
              </Button>}
        ...
        <Button onClick={handleSubmit} disabled={selectedDates.length === 0 || isEmergencyBooking && timeSlots.length === 0} className="w-full mt-6 py-4 text-lg font-semibold rounded-full bg-purple-600 hover:bg-purple-700 text-white disabled:opacity-50">
