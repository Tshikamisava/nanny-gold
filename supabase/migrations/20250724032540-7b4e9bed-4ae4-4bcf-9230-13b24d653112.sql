...
BEGIN
  -- Calculate authorization date for current month
  current_month_auth := date_trunc('month', current_date) + INTERVAL '1 month' - INTERVAL '1 day' - 
                       INTERVAL '1 day' * (EXTRACT(DAY FROM (date_trunc('month', current_date) + INTERVAL '1 month' - INTERVAL '1 day')) - auth_day);
  
  -- Calculate capture date for next month
  current_month_capture := date_trunc('month', current_date) + INTERVAL '1 month' + INTERVAL '1 day' * (capture_day - 1);
  
  -- If we've passed the current month's auth date, move to next month
  IF current_date > current_month_auth THEN
    next_auth_date := date_trunc('month', current_date) + INTERVAL '1 month' + INTERVAL '1 day' * (auth_day - 1);
    next_capture_date := date_trunc('month', current_date) + INTERVAL '2 months' + INTERVAL '1 day' * (capture_day - 1);
  ELSE
