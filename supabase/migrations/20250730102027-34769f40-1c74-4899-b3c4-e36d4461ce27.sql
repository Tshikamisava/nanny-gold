-- First, let's see what's happening with the handle_new_user trigger
-- Let's fix the phone storage issue and improve error handling
...
-- Clean up unused OTP codes to prevent confusion
UPDATE public.temp_otp_codes SET used = true WHERE expires_at < now();