-- Fix ambiguous column references in get_user_chat_rooms function
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(cr.name, 
      CASE 
        ...
        ELSE CONCAT(profiles.first_name, ' ', profiles.last_name)
      END
    ) as room_name,
    other_participants.user_id as other_participant_id,
    CONCAT(profiles.first_name, ' ', profiles.last_name) as other_participant_name,
  ...
  LEFT JOIN public.profiles ON profiles.id = other_participants.user_id
  LEFT JOIN LATERAL (
