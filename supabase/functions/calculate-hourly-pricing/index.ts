...
Deno.serve(async (req) => {
  ...
  try {
    // Supabase client removed as it was unused
    ...
    const safetyTotalHours = totalHours || 0;
    if (services.specialNeeds) {
      const hourlyRate = servicePricing.specialNeeds;
      serviceCharges.push({
        name: 'Diverse Ability Support',
        hourlyRate,
        totalCost: hourlyRate * safetyTotalHours
      });
    ...
    let subtotal = effectiveHourlyRate * safetyTotalHours;
    ...
    const finalHourlyRate = safetyTotalHours > 0 ? total / safetyTotalHours : 0;
